...
```
payload ThreadProc:  OpenEvent("SwitchLitePayloadDone") -> do JNI work -> SetEvent
injector:            CreateEvent("SwitchLitePayloadDone") -> inject DLL -> WaitForSingleObject(event, 15s) -> check result
```
...
```cpp
HANDLE hDone = OpenEventA(EVENT_MODIFY_STATE, FALSE, "SwitchLitePayloadDone");
```
...
```cpp
HANDLE hDone = CreateEventA(NULL, TRUE, FALSE, "SwitchLitePayloadDone");
...
- **`agent/src/main/java/io/switchlite/agent/Agent.java`** — DO NOT TOUCH. R key polling, file logging, bootstrap logic all stay as-is.
...
- payload.cpp: signal SwitchLitePayloadDone event when ThreadProc completes
