...
std::string configPath = std::string(configDir) + "\\switchlite-config.properties";
...
if (g_isForge) {
    ...
    if (cfg) {
        fprintf(cfg, "switchlite.platform=Forge\n");
        fprintf(cfg, "switchlite.version=1.8.9\n");  // preserve version from original
        fclose(cfg);
        payloadLog("[SwitchLite] Config updated: platform=Forge (detected Launch class)");
    }
}
```

Wait — we don't have the version string in ThreadProc. Better approach:
just append/overwrite the platform line only. Read the file, find "switchlite.platform=",
...
if (g_isForge) {
    ...
    size_t pos = content.find("switchlite.platform=");
    if (pos != std::string::npos) {
        size_t lineEnd = content.find('\n', pos);
        if (lineEnd == std::string::npos) lineEnd = content.length();
        content.replace(pos, lineEnd - pos, "switchlite.platform=Forge");
    }
    // Write back
    std::ofstream out(configPath);
    out << content;
    out.close();
    payloadLog("[SwitchLite] Config updated: platform=Forge");
}
```
...
if (g_isForge) {
    ...
    if (f) {
        ...
        char* pos = strstr(buf, "switchlite.platform=");
        if (pos) {
            char* lineEnd = strchr(pos, '\n');
            size_t restLen = lineEnd ? (lineEnd - pos) : (strlen(pos));
            char replacement[] = "switchlite.platform=Forge";
...
- **`agent/src/main/java/io/switchlite/agent/Agent.java`** — DO NOT TOUCH
...
[AGENT] [Chat] Sent: [SwitchLite] Agent injected! Press R for GUI  ← fixed!
[AGENT] [KeyPoll] Thread started, polling LWJGL2 Keyboard for R key
```

Then pressing R in-game should show `[SwitchLite] GUI: ON/OFF` in chat.
