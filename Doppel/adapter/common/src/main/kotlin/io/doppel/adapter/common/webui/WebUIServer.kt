...
import io.doppel.adapter.common.option.OptionDescriptor
import io.doppel.adapter.common.option.OptionType
...
object WebUIServer {
        ...
        synchronized(lock) {
            ...
            try {
                ...
                srv.executor = Executors.newFixedThreadPool(2) { r ->
                    Thread(r, "Doppel-WebUI").apply { isDaemon = true }
                }
                ...
                try {
                    ...
                    val pw = java.io.PrintWriter(
                        java.io.FileWriter(java.io.File(temp, "doppel-payload.log")), true)
    ...
    private fun handleAll(exchange: HttpExchange) {
        try {
            ...
            if (!authorized(exchange)) {
                exchange.responseHeaders.add("WWW-Authenticate", "$AUTH_SCHEME realm=\"doppel-webui\"")
        ...
        try {
            ...
            when {
                ...
                path.endsWith("/hud") && name.isNotBlank() -> {
                    ...
                    try {
                        io.doppel.adapter.common.module.render.HUD.notifyModuleToggled()
                    } catch (_: Throwable) {}
                    try { ConfigStore.save() } catch (_: Exception) {}
                    respondJson(exchange, 200, mapper.writeValueAsString(
    ...
    private fun handleStatic(exchange: HttpExchange) {
        try {
            val path = exchange.requestURI.path
            val resource = when {
                path == "/" || path.isEmpty() -> "/doppel/webui/index.html"
                else -> "/doppel/webui${if (path == "/") "/index.html" else path}"
            }
    ...
    private fun moduleJson(module: Module): Map<String, Any?> {
        ...
        return linkedMapOf(
            ...
            "hudEnabled" to (module is io.doppel.adapter.common.module.HudLineProvider),
            "options" to options
        )
    }
    ...
    fun keyName(code: Int): String {
        ...
        return when (code) {
            io.doppel.adapter.common.api.KeyCode.LEFT_SHIFT -> "左Shift"
            io.doppel.adapter.common.api.KeyCode.RIGHT_SHIFT -> "右Shift"
            io.doppel.adapter.common.api.KeyCode.LEFT_CONTROL -> "左Ctrl"
            io.doppel.adapter.common.api.KeyCode.RIGHT_CONTROL -> "右Ctrl"
            io.doppel.adapter.common.api.KeyCode.LEFT_ALT -> "左Alt"
            io.doppel.adapter.common.api.KeyCode.RIGHT_ALT -> "右Alt"
            io.doppel.adapter.common.api.KeyCode.SPACE -> "空格"
            io.doppel.adapter.common.api.KeyCode.ENTER -> "回车"
            io.doppel.adapter.common.api.KeyCode.TAB -> "Tab"
            io.doppel.adapter.common.api.KeyCode.ESC -> "Esc"
            in 48..57 -> "键${(code - 48)}"
