...
| **Netty 网络线程** | `ForgePacketInterceptor.channelRead()` | 拦 `S12PacketEntityVelocity`/`S27PacketExplosion` → `EventBridge.onVelocityPacket` |
...
`agent/Transformer.java` 只 hook `Display.update()`（渲染管线）。攻击方法注入（KeepSprint 的字节码方案）已废弃，见 KeepSprint 模块注释。
...
| 想改什么 | 看哪个文件 |
|---|---|
| 加一个新模块 | `adapter/common/module/<category>/X.kt` + `ForgeBootstrap.registerAll()` 注册 + `OverlayRenderer`（如 render 类） |
