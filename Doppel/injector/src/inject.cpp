...
std::string getEmbeddedAgentPath() {
    ...
    std::string agentPath = std::string(tempPath) + "doppel-agent.jar";
...
std::string getEmbeddedPayloadPath() {
    ...
    char tempPath[MAX_PATH];
    GetTempPathA(MAX_PATH, tempPath);
    std::string dllPath = std::string(tempPath) + "doppel-payload.dll";
...
std::string getFabricModPath() {
    return "./adapter/fabric/build/libs/Doppel-Fabric.jar";
}
...
bool injectJavaAgent(int pid, const std::string& agentPath, const VersionInfo& versionInfo) {
    ...
    std::string configPath = configDir + "\\doppel-config.properties";
    std::ofstream cfg(configPath);
    if (cfg.is_open()) {
        cfg << "doppel.platform=" << versionInfo.platform << std::endl;
        cfg << "doppel.version=" << versionInfo.version << std::endl;
        cfg.close();
    ...
    std::string eventName = "DoppelPayloadDone_" + std::to_string(pid);
...
void showDiagnosticLogs() {
    char tempPath[MAX_PATH];
    GetTempPathA(MAX_PATH, tempPath);
    std::string payloadLog = std::string(tempPath) + "doppel-payload.log";
    std::string agentLog = std::string(tempPath) + "doppel-agent.log";
